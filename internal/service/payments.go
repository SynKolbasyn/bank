...
func NewPayments(repositoryPayments repository.IPayments, notificationManager INotificationManager) *Payments {
	return &Payments{
...
func (p *Payments) Create(ctx context.Context, userID uuid.UUID, payment model.PaymentRequest) error {
	if userID == payment.RecipientID {
		return domain.NewAppError(http.StatusBadRequest, errors.New("userID == payment.RecipientID"))
	}
